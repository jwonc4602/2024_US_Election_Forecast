...
The main variable we aim to forecast is the pct variable, which represents the proportion of the vote a candidate received in a poll.@tbl-pct_cleaned and @fig-pct_cleaned shows the summary statistics and distribution of pct variable in a filtered dataset that only comprises of votes from relatively high-quality polling organizations. We also get the popular vote predictions for each candidate using the predict() function and add this to the dataset as the predicted_pct variable. We have showed the summary statistics and distribution for predicted_pct in @tbl-pct_predicted and @fig-pct_predicted, respectively.
...
```{r}
#| label: tbl-pct_cleaned
#| fig-cap: Summary statistics for the proportion of support candidates received in the poll
#| echo: false
#| tbl-align: center
#| tbl-width: 82%
...
```{r}
#| label: tbl-pct_predicted
#| fig-cap: Summary statistics for the predicted proportion of support candidates received in the poll
#| echo: false
#| tbl-align: center
#| tbl-width: 82%
...
Poll Regency (old if data collected in 30 days, old to capture regent old
